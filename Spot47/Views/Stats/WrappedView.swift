...
struct WrappedView: View {
    ...
    private var pages: [WrappedPage] {
        ...
        result.append(.stat(
            ...
            "sightings this \(monthName)",
            "eye.fill",
            .blue
        ))

        // Page 3: Verified count
        let verified = monthSightings.filter(\.contains47).count
        if verified > 0 {
            result.append(.stat(
                "\(verified)",
                "OCR-verified finds",
        ...
        if let top = catCounts.first {
            result.append(.stat(
                top.key.capitalized,
                "was your top category with \(top.value) sighting\(top.value == 1 ? "" : "s")",
        ...
        result.append(.stat(
            "\(yearSightings.count)",
            "sightings in \(currentYear) so far",
            "calendar",
            .indigo
        ))
