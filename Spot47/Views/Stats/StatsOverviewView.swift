...
struct StatsOverviewView: View {
    ...
    private var memoriesCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("See your past sightings from \(Date.now.formatted(.dateTime.month(.wide).day()))")
                .font(.caption)
                .foregroundStyle(.secondary)
        }
        .padding()
    ...
    private var challengesCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if activeChallenges.isEmpty {
                Text("No active challenges — check back soon!")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            } else {
    ...
    private var achievementsCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("Track your progress and unlock badges")
                .font(.caption)
                .foregroundStyle(.secondary)
        }
        .padding()
    ...
    private var mapPreviewCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                Label("Sighting Map", systemImage: "map.fill")
                    .font(.headline)
                Spacer()
                Image(systemName: "chevron.right")
                    .font(.caption)
