...
struct LibraryScannerView: View {
    ...
    private var preScanView: some View {
        VStack(spacing: 24) {
            ...
            Text("Find 47s in Your Photos")
                .font(.title2.bold())

            Text("Scan your photo library to discover existing photos that contain the number 47.")
                .font(.subheadline)
                .foregroundStyle(.secondary)
        ...
        VStack(spacing: 16) {
            ...
            if scanner.matches.isEmpty {
                ContentUnavailableView {
                    ...
                    Text("Looking for 47s in your photos")
                }
        ...
        VStack(spacing: 0) {
            if scanner.matches.isEmpty {
                VStack(spacing: 24) {
                    ...
                    Text("No 47s Found")
                        .font(.title2.bold())
                    Text("None of your photos contained the number 47. Try capturing some with the camera!")
                        .font(.subheadline)
                        .foregroundStyle(.secondary)
                        .multilineTextAlignment(.center)
                        .padding(.horizontal, 40)
    ...
    private var resultsHeader: some View {
        HStack {
            Text("\(scanner.matches.count) photo\(scanner.matches.count == 1 ? "" : "s") with 47")
                .font(.headline)
            Spacer()
    ...
    private func importSelected() async {
        ...
        for result in toImport {
            ...
            do {
                ...
                sighting.detectedText = result.ocrResult.fullText
                sighting.contains47 = result.ocrResult.contains47
